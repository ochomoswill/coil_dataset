...
dataset_eval = pd.read_csv("{}/dataset/ticeval2000.txt".format(os.getcwd()),sep='\s+')
dataset_tgts = pd.read_csv("{}/dataset/tictgts2000.txt".format(os.getcwd()),sep='\s+')
...
cleaned_dataset_eval = preprocess_dataset(dataset_eval)

# Describe the new dataset
cd_describe = cleaned_dataset.describe()
cde_describe = cleaned_dataset_eval.describe()

# Matrix of features
X_train = cleaned_dataset.iloc[:,:-1].values
y_train = cleaned_dataset.iloc[:,-1].values

# Splitting the dataset into training set and test set
# from sklearn.model_selection import train_test_split
# X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=0.2, random_state=0)

X_test = cleaned_dataset_eval.iloc[:,:].values
y_test = dataset_tgts.iloc[:,:].values

#This data is already label encoded so only HotEncode
# from sklearn.preprocessing import OneHotEncoder
# one_hot_encoder_1 = OneHotEncoder(categorical_features=[0,4,9,15])
# X_train = one_hot_encoder_1.fit_transform(X_train)
# one_hot_encoder_2 = OneHotEncoder(categorical_features=[0,4,9,15])
# X_test = one_hot_encoder_2.fit_transform(X_test)
